...
*ALT: bar graph showing the growth in the number of lessons from 2015 to present
day. There is an arrow pointing to the top of the data that says "139 lessons".
The data are stratified by our four programs: Software Carpentry on bottom,
followed by Data Carpentry, Library Carpentry, and Incubator. The incubator 
comprises of more than one half of all lessons in the current day.*
...
---
background-image: url(https://bookdown.org/yihui/blogdown/images/logo.png), url(unique-brush.jpg)
...
templates, which has been done many times. 
...
infrastructure by creating three R packages: 
 ...
 - {sandpaper} is the engine that orchestrate building and maintaining lessons
 - {varnish} is the styling that hosts the CSS, JS, and HTML templates
...
the Alfred P. Sloan Foundation, the Moore Foundation, the Chan-Zuckerberg Initiative, and the R Consortium
