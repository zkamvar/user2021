...
that we align all of our decisions with our core values, putting people first
and accepting all contributions.
...
The source for these lessons are hosted on GitHub where we have a core set of
volunteer maintainers to make sure that the information on lessons is
...
further our mission**. In this talk, I will introduce you to our current
...
have continuously updated our workflows to lower barriers to contributors. In The Carpentries, you
...
the Jekyll static site generator. This was chosen because it was
the most straightforward way to create a static website without needing a server
like wordpress or drupal sites. Ideally, it also provided an example of how people could build their own websites.

It's important to pause here and point out that this entire paradigm is
for people to be able to write markdown lessons and get a functional website. 
This is not a new concept, in fact, there are over 460 iterations of
...
2. Another drawback is that we have a lesson website wrapped around a static
   site generator (which itself is a kind of cowpath). This meant
   that it was easy to contribute if you were familiar with how Jekyll
   operated. Some would say Jekyll is well documented, but in my experience,
   Jekyll is well, documented. There was often a moment of panic in a new
   contributor's eyes when you showed them what a lesson repository looks like.
   When looking at the git repository, it was often unclear where to even start,
   because of all the exposed wiring
   with no signs that clearly say "start here". This aspect has lead to several
   lessons being built in _slightly_ different ways.

This brings me back to the cowpaths. Over the last few years, they've begun popping 
up all across the lesson infrastructure,
appearing through issues, pull requests and general
...
We have seen not only an increase in the number of community members, but also,
with the carpentries incubator for community contributed lessons,
a massive increase in the number of lessons using the template.
...
infrastructure that scales better and separates the tools from the content.
...
Eventually, we settled on R as the best option for several reasons:
...
to build everything live in a separate package, and it can be customized by
making your own package to host your CSS, JS, and HTML templates.

From this model, we created three packages to handle infrastructure tools,
...
dropdown menus, so our lessons consist of folders that we commonly use: one
folder for lesson chapters, one for extra information for learners, one for
extra information for instructors, and one that contains learner profiles. The
...
Of course, our solution ticks all the boxes for our design choices, 
but we need to make sure it isn't something only R enthusiasts will
love. If it is unusable to newcomers, Python users, or Matlab folks, then we are not
...
where people can put their markdown/RMarkdown/MyST notebooks in a folder and
...
template to see if people stumbled at any particular points. I want to take a
...
on a few live lessons to make sure it works for the community. It's a slow
...
I want to conclude by saying that we ended up choosing a solution that we
